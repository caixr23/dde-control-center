...
#define LABEL_SOURCETEXT "sourceText"
...
void SwitchLabel::resizeEvent(QResizeEvent *event)
{
    ...
    if(len > m_actualSize.width()) {
        ...
        this->setText(property(LABEL_SOURCETEXT).toString());
    }
    ...
    , m_switchBtn(new DSwitchButton)
{
    ...
    m_leftWidget->setProperty(LABEL_SOURCETEXT, title);
    init();
}
...
void SwitchWidget::init()
{
    ...
    connect(m_switchBtn, &DSwitchButton::toggled, this, [this](bool checked) {
...
void SwitchWidget::setTitle(const QString &title)
{
    ...
    if (label) {
        ...
        label->setText(title);
        label->setProperty(LABEL_SOURCETEXT, title);
        label->setWordWrap(true);
    }
...
bool SwitchWidget::event(QEvent *event)
{
    ...
    return SettingsItem::event(event);
}
