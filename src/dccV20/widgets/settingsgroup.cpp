...
void SettingsGroup::removeItemAndNotDestruct(SettingsItem *item)
{
    if (!item)
        return;
...
SettingsItem *SettingsGroup::searchItem(const QString &title)
{
    if (!m_bggroup)
        return nullptr;

    return m_bggroup->findChild<SettingsItem *>(title, Qt::FindDirectChildrenOnly);
}
